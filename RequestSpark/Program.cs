...
using RequestSpark.Domain.Services;
...
.ConfigureServices((hostContext, services) =>
{
    ...
    services.AddSingleton(serviceProvider =>
    {
        ...
        var baseDirectoryCollectionPath = Path.Combine(AppContext.BaseDirectory, "collection.json");
        var workingDirectoryCollectionPath = Path.Combine(Directory.GetCurrentDirectory(), "collection.json");
        var collectionPath = File.Exists(baseDirectoryCollectionPath)
            ? baseDirectoryCollectionPath
            : workingDirectoryCollectionPath;

        if (!File.Exists(collectionPath))
        {
            throw new FileNotFoundException(
                $"Could not find collection.json. Checked '{baseDirectoryCollectionPath}' and '{workingDirectoryCollectionPath}'.");
        }

        import.LoadFromPostman(collectionPath);
        myRunner.SaveToFile();
        return myRunner;
    });
...
using (var serviceScope = host.Services.CreateScope())
{
    ...
    try
    {
        Console.WriteLine("Starting RequestSpark execution...");
        Console.WriteLine($"Start Time: {DateTime.Now:yyyy-MM-dd HH:mm:ss}");

        var myService = services.GetRequiredService<IExecuteRunner>();
        var statistics = await myService.ExecuteRunnerAsync(new CsvOutput($"c:\\test\\RequestSpark.csv")).ConfigureAwait(false);
...
Console.WriteLine("\n✅ RequestSpark execution completed successfully!");
return 0;

static void DisplayExecutionStatistics(ExecutionStatistics statistics)
{
    Console.WriteLine("\n" + new string('=', 80));
    Console.WriteLine("🚀 REQUESTSPARK EXECUTION STATISTICS");
    ...
    Console.WriteLine(new string('=', 80));
    Console.WriteLine($"📄 Results exported to: c:\\test\\RequestSpark.csv");
