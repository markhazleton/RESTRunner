...
namespace RequestSpark.Domain.Services;
    ...
    ILogger<ExecuteRunnerService> logger) : IExecuteRunner
{
    ...
    public async Task<ExecutionStatistics> ExecuteRunnerAsync(IOutput output, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            var sink = new RequestSparkBatchSink(output, stats, compareRunner.SessionId);

            logger.LogInformation(
                "Starting RequestSpark batch execution with {InstanceCount} instances, {UserCount} users, {RequestCount} requests, {Iterations} iterations",
        ...
        {
            logger.LogInformation("RequestSpark execution was cancelled");
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Unexpected error during RequestSpark execution");
        }
        finally
        {
    ...
    private sealed class RequestSparkBatchSink(IOutput output, ExecutionStatistics stats, string? sessionId) : IBatchExecutionResultSink
    {
